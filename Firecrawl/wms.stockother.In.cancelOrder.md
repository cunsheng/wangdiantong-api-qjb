...
| JSON | |     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15 | `{`<br>```"status"``: 0,`<br>```"data"``: {`<br>```"success_data"``: [`<br>```"QR3796"``,`<br>```"QR3795"`<br>```],`<br>```"error_data"``: [`<br>```{`<br>```"order_no"``:``"QR3797"``,`<br>```"err_msg"``:``"单据状态不是编辑中、待审核、已审核，不允许取消"`<br>```}`<br>```]`<br>```}`<br>`}` | |
