...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15 | `{`<br>```"status"``:0,`<br>```"data"``:{`<br>```"error_info"``:[`<br>```{`<br>```"order_no"``:``"xxxxxxx"``,`<br>```"message"``:``"采购退货单号不存在"`<br>```},`<br>```{`<br>```"order_no"``:``"lskdjlkjklj"``,`<br>```"message"``:``"采购退货单号不存在"`<br>```}],`<br>```"status"``:0`<br>```}`<br>`}` |
