---
url: "https://open.wangdian.cn/qjb/open/apidoc/doc?path=wms.stockin.Base.searchSN"
title: "API文档"
---
...
**wms.stockin.Base.searchSN（入库SN查询** **）**
...
| 名称 | 字段 | 类型 | 长度 | 必须 | 描述 |
| --- | --- | --- | --- | --- | --- |
| 开始时间 | start\_time | String |  | Y | 入库SN记录创建的时间<br>yyyy-MM-dd HH:mm:ss格式 |
| 结束时间 | end\_time | String |  | Y | 入库SN记录创建的时间<br>yyyy-MM-dd HH:mm:ss格式 |
...
| 名称 | 字段 | 类型 | 长度 | 必须 | 描述 |
| --- | --- | --- | --- | --- | --- |
| 入库sn记录id | rec\_id | Int | 11 | Y | 入库sn记录id，不是sn的id |
| 入库单id | stockin\_id | Int | 11 | Y | 入库单id |
| 入库单号 | stockin\_no | String | 40 | Y | 入库单号 |
| 入库单明细id | stockin\_detail\_id | Int | 11 | Y | 入库单明细id |
| 单品id | spec\_id | Int | 11 | Y | 单品id |
| 商家编码 | spec\_no | String | 40 | Y | 商家编码 |
| sn | sn | String | 80 | Y | sn |
| 是否残次品 | defect | boolean |  | Y | 入库单明细记录的正残品属性 |
| 创建时间 | created | String |  | Y | 入库SN记录创建的时间，yyyy-MM-dd HH:mm:ss格式 |
...
| php 请求示例 | |     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13 | `<?php`<br>`header(``"Content-Type: text/html; charset=UTF-8"``);`<br>`date_default_timezone_set(``"Asia/Shanghai"``);`<br>`$client``=``new``WdtErpClient(``"url"``,``"wdtapi3"``,``"appkey"``,``"secret"``)`<br>``<br>`$params``=``new``stdClass();`<br>``<br>`$params``->start_time =``'2023-06-31 15:56:14'``;`<br>`$params``->end_time =``'2023-07-30 16:56:14'``;`<br>``<br>`$pager``=``new``Pager(1, 0, true);`<br>`$data``=``$client``->pageCall(``"wms.stockin.Base.searchSN"``,``$pager``,``$params``);?>` | |
| JAVA |  |
| C# |  |
...
| json | |     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>26<br>27<br>28<br>29<br>30 | `{`<br>```"data"``: {`<br>```"detail_list"``: [`<br>```{`<br>```"created"``:``"2023-07-03 10:06:00"``,`<br>```"defect"``:``false``,`<br>```"rec_id"``: 7047,`<br>```"sn"``:``"ytzxhl2022001"``,`<br>```"spec_id"``: 2264,`<br>```"spec_no"``:``"ytzxlh"``,`<br>```"stockin_detail_id"``: 33402,`<br>```"stockin_id"``: 15412,`<br>```"stockin_no"``:``"RK202307030002"`<br>```},`<br>```{`<br>```"created"``:``"2023-07-03 10:44:09"``,`<br>```"defect"``:``false``,`<br>```"rec_id"``: 7048,`<br>```"sn"``:``"ytzxhl2022001"``,`<br>```"spec_id"``: 2264,`<br>```"spec_no"``:``"ytzxlh"``,`<br>```"stockin_detail_id"``: 33404,`<br>```"stockin_id"``: 15414,`<br>```"stockin_no"``:``"RK202307030004"`<br>```}`<br>```],`<br>```"total_count"``: 46`<br>```},`<br>```"status"``: 0`<br>`}` | |
