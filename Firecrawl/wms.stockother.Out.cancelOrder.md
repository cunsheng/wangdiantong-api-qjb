...
| JSON | |     |     |
| --- | --- |
|  | `{`<br>```"status"``: 0,`<br>```"data"``: {`<br>```"success_data"``: [`<br>```"QC202409200002"`<br>```],`<br>```"error_data"``: [{``"order_no"``:``"QC202409200002"``,``"err_msg"``:``"单据状态不是编辑中、待审核、已审核，不允许取消"``},{``"order_no"``:``"QC202409200001"``,``"err_msg"``:``"单据状态不是编辑中、待审核、已审核，不允许取消"``}]`<br>```}`<br>`}` | |
