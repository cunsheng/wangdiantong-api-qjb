...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19 | `{`<br>```"status"``: 0,`<br>```"data"``: {`<br>```"total_count"``: 14,`<br>```"order_list"``: [`<br>```{`<br>```"roles_mask"``: 0,`<br>```"gender"``: 0,`<br>```"is_system_user"``:``false``,`<br>```"employee_id"``: 374,`<br>```"name"``:``""``,`<br>```"position_name"``:``"慧策技术支持"``,`<br>```"employee_no"``:``"lxxx-test2"``,`<br>```"remark"``:``"开放接口"``,`<br>```"shortname"``:``"lxxxx-test2"`<br>```}`<br>```]`<br>```}`<br>`}` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4 | `{`<br>```"status"``: 100,`<br>```"message"``:``"超过每分钟最大调用频率限制,请稍后重试"`<br>`}` |
