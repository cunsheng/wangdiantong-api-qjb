...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `[`<br>```[{`<br>```"spec_no"``:``"dongfangshuye"``,`<br>```"provider_no"``:``"LCJtest"`<br>```}, {`<br>```"spec_no"``:``"wangdiantong"``,`<br>```"provider_no"``:``"20170808002"`<br>```}]`<br>`]` |

|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14 | `<?php`<br>`header(``"Content-Type: text/html; charset=UTF-8"``);`<br>`date_default_timezone_set(``"Asia/Shanghai"``);`<br>``<br>`$client``=``new``WdtErpClient(``"url"``,``"wdtapi3"``,``"appkey"``,``"secret"``);`<br>``<br>`$parMap``=``new``StdClass();`<br>``<br>`$parMap``->provider_no =’1001’;`<br>`$parMap``->spec_no =’FAQB06201M’;`<br>``<br>`$data``=``$client``->call(``"purchase.ProviderGoods.upload"``,[``$parMap``]);`<br>``<br>`?>` |
...
|     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9 | `{`<br>```"status"``: 0,`<br>```"data"``: {`<br>```"error_list"``: [`<br>```"LCTtest11 供应商货品重复,商家编码：dongfangshuye"``,`<br>```"秋天001 供应商货品重复,商家编码：wangdiantong"`<br>```]`<br>```}`<br>`}` |
