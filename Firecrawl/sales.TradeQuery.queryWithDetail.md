...
| 是否强制指定src\_tid精准查询 | accurate\_query | boolean | 1 | n | 0：否<br>1：是<br>默认0 |
| 是否截取物流单号 | cut\_logistics\_no | boolean | 1 | n | 0：不截取<br>1：截取<br>默认0 |
...
| 平台支付帐号 | pay\_account | String | 128 | y | 平台支付帐号, （仅自有平台及线下平台返回，其他平台均不返回） |
...
| 客户网名 | buyer\_nick | String | 100 | y | 客户网名（淘宝ouid,淘系平台不返回）, （仅自有平台及线下平台返回，其他平台均不返回） |
| 收货人/收件人 | receiver\_name | String | 100 | y | 收货人/收件人, （仅自有平台及线下平台返回，其他平台均不返回） |
...
| 地址 | receiver\_address | String | 256 | y | 收件人地址, （仅自有平台及线下平台返回，其他平台均不返回） |
| 手机 | receiver\_mobile | String | 40 | y | 手机,（仅自有平台及线下平台返回，其他平台均不返回） |
| 固话 | receiver\_telno | String | 40 | y | 固话, （仅自有平台及线下平台返回，其他平台均不返回） |
...
| 赠品标记 | gift\_mask | Int | 4 | y | 赠品标记<br>0：无标记<br>1：自动赠送<br>2：手工赠送<br>4：回购赠送<br>8：平台赠送<br>（注意：如果是3，则表示既有自动赠送也有手工赠送“1+2”） |
...
| 平台标签 | platform\_label | String |  | y | 平台标签（客户端V1.5.6.8及以上版本返回） |
| 分销商编号 | fenxiao\_nick\_no | String |  |  | 分销商编号 |
...
| 订单内部来源扩展 | from\_mask\_ext | Int | 11 | y | 订单内部来源:<br>2：小时达订单<br>4：自选物流<br>16：国补订单 |
...
| 图片路径 | img\_url | String | 1024 | y | 图片路径 |
...
| 赠品关联关系 | goods\_gift\_relation | String |  | y | 赠品关联关系  返回执行的策略编号和名称 |
| 成本价 | avg\_cost | Decimal(19,4) |  | y | 成本价 |
| 已占用库存 | reserve\_num | Decimal(19,4) |  | y | 已占用库存 |

**5.请求示例**

- [JSON](https://open.wangdian.cn/qjb/open/apidoc/doc?path=sales.TradeQuery.queryWithDetail#autoTab0_0)
- [PHP](https://open.wangdian.cn/qjb/open/apidoc/doc?path=sales.TradeQuery.queryWithDetail#autoTab0_1)
- [JAVA](https://open.wangdian.cn/qjb/open/apidoc/doc?path=sales.TradeQuery.queryWithDetail#autoTab0_2)
- [C#](https://open.wangdian.cn/qjb/open/apidoc/doc?path=sales.TradeQuery.queryWithDetail#autoTab0_3)
