...
|     |     |
| --- | --- |
|  | `[`<br>```{`<br>```"outer_no"``:``"CG2019112800061234567890"``,`<br>```"warehouse_no"``:``"ytz"``,`<br>```"goods_list"``: [`<br>```{`<br>```"spec_no"``:``"gy01"``,`<br>```"num"``:``"3"``,`<br>```"stockin_price"``: 123,`<br>```"sn_list"``:[``"sn001"``,``"sn002"``,``"sn003"``]`<br>```}`<br>```]`<br>```}`<br>`]` |
...
|     |     |
| --- | --- |
|  | `{`<br>```"status"``: 0,`<br>```"data"``: {`<br>```"message"``:``"ljcs39423423422"``,`<br>```"status"``: 0`<br>```}`<br>`}` |
