...
| Json格式请求报文 | |     |     |
| --- | --- |
|  | `[`<br>```{`<br>```"province_id"``:``"110000"`<br>```}`<br>`]` | |
| PHP | |     |     |
| --- | --- |
...
| JSON | |     |     |
| --- | --- |
|  | `{`<br>```"data"``: {`<br>```"detail_list"``: [`<br>```{`<br>```"city_id"``: 110100,`<br>```"modified"``:``"2022-03-08 10:16:02"``,`<br>```"name"``:``"北京市"``,`<br>```"province_id"``: 110000`<br>```}`<br>```],`<br>```"total_count"``: 1`<br>```},`<br>```"status"``: 0`<br>`}` | |
...
| JSON | |     |     |
| --- | --- |
|  | `{`<br>```"message"``:``"此方法仅支持分页查询"``,`<br>```"status"``: 100`<br>`}` | |
