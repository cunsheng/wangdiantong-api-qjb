---
url: "https://open.wangdian.cn/qjb/open/apidoc/doc?path=setting.Address.citySearch"
title: "API文档"
---
...
**setting.Address.citySearch（地** **址查询-市** **）**
...
| **1.5注意事项：** 查询需要传入省id或者起止时间，省id通过省查询接口获取 |
...
| JSON | |     |     |
| --- | --- |
| 1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14 | `{`<br>```"data"``: {`<br>```"detail_list"``: [`<br>```{`<br>```"city_id"``: 110100,`<br>```"modified"``:``"2022-03-08 10:16:02"``,`<br>```"name"``:``"北京市"``,`<br>```"province_id"``: 110000`<br>```}`<br>```],`<br>```"total_count"``: 1`<br>```},`<br>```"status"``: 0`<br>`}` | |
